...
//    filterAcc(p, new Empty).union(left.filter(p)).union(right.filter(p))
//  }
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    else that.filterAcc((x: Tweet) => true, this)
  }

  def mostRetweeted: Tweet = {
    def compare(t: Tweet, t2: Tweet): Tweet = if (t2.retweets > t.retweets) t2 else t
...
object Zero extends Nat {
  ...
  def - (that: Nat): Nat = throw new Exception("Negative number. ")
...
class Succ(n: Nat) extends Nat {
  ...
  def + (that: Nat): Nat = if (that.isZero) this else this + that.predecessor
